...
int pythonPredictionCode(float* X, float *Y);
...
unsigned pMoveCell(Inst *Cell, unsigned rowBegIdx, unsigned rowEndIdx,
				   ...
				   unsigned mCellGridRow, unsigned mCellGridCol, ofstream &logs,unsigned &numCellsMoved,bool &hasMoved,unsigned &wirelengthFlag);
...
int generateOutputDataSampleCurrentCell(ofstream &dataFile, unsigned rowStartIndex, unsigned rowEndIndex, unsigned colStartIndex, unsigned colEndIndex,
										 ...
										 pinIndexes *pinIndexList, list<Inst *> &movableCellList, unsigned k, unsigned numNetsCurrCell,ofstream &logs,unsigned &numCellsMoved,unsigned &numPredictionTrials);
int partitionDesignClustered(vector<Inst *> &instList, ofstream &dataFile, unsigned rowStartIndex, unsigned colStartIndex, unsigned rowEndIndex, unsigned colEndIndex,
							 unsigned startGridRowIdx, unsigned endGridRowIdx, unsigned startGridColIdx, unsigned endGridColIdx,
							 list<Inst *> &cellsInWindow, list<Inst *> &cellsInRowWindow, list<Inst *> &cellsBeyondRowWindow,ofstream &logs,unsigned &numCellsMoved,unsigned &numPredictionTrials);
...
int main(int ac, char **av)
{
	...
	//unsigned rowStep = getStepForWindow(gv.rowEndIdx, 5);
	//unsigned colStep = getStepForWindow(gv.colEndIdx, 5);
	...
	time(&start);
	slidingWindowScan(gv.dd.instList,gv.rowBeginIdx,gv.colBeginIdx,gv.rowEndIdx,gv.colEndIdx,10,10,rowStep,colStep,logs,numCellsMoved,numPredictionTrials);
	//slidingWindowScan(gv.dd.instListFirstPartition, firstPartitionStart, gv.colBeginIdx, firstPartitionEnd,
	//gv.colEndIdx, 10 ,10, rowStep, colStep, numCellsMovedFirstPartition);
	
	/*logs0.open("logs0.txt");
	logs1.open("logs1.txt");
	logs2.open("logs2.txt");
	logs3.open("logs3.txt");
	logs4.open("logs4.txt");
	logs5.open("logs5.txt");
	logs6.open("logs6.txt");
	logs7.open("logs7.txt");*/
	...
	cout << "numMovedCells = " << numCellsMoved/* numCellsMovedFirstPartition + numCellsMovedSecondPartition*//*+numCellsMovedThirdPartition + numCellsMovedFourthPartition+numCellsMovedFifthPartition+numCellsMovedSixthPartition+numCellsMovedSeventhPartition+numCellsMovedEighthPartition*/<< endl;
	...
	cout << "numPredictionTrials = " <<numPredictionTrials/* numPredictionTrialsSecondPartition + numPredictionTrialsFirstPartition*//* + numPredictionTrialsThirdPartition +numPredictionTrialsFourthPartition +numPredictionTrialsFifthPartition+numPredictionTrialsSixthPartition+numPredictionTrialsSeventhPartition+numPredictionTrialsEighthPartition*/<< endl;
	produceRouterLogs();
			...
			while (getline(ss, tok, ' '))
			{
				...
				if (tok == "GGridBoundaryIdx")
				{
					...
					// ! DOUBT: Why 8 parts?
					{
						...
						thirdPartitionEnd = (gv.rowEndIdx / 8) * 3;
						fourthPartitionStart = thirdPartitionEnd + 1;
						fourthPartitionEnd = (gv.rowEndIdx /8)*4;
				...
				if (currentNet && (tok == "Pin"))
				{
					...
					// pin->inst->adjacencyList --> We would want to add all the cells containing the connected pin to this pin.

					/*list<Inst *>::iterator netCellItr = netCellList.begin(); // ! ERROR => We are adding unconnected cells into the connectivity
					for (; netCellItr != netCellList.end(); netCellItr++)
					{
						inst->adjacencyList.push_back(*netCellItr); //non unique elements may be pushed, but OK
						(*netCellItr)->adjacencyList.push_back(inst);
					}*/
...
int generateDemandSuppVals()
{
	...
	for (z = 1; z <= gv.numLayers; z++)
	{
		for (x = gv.rowBeginIdx; x <= gv.rowEndIdx; x++)
		{
			...
			{
				//logs<<"\t("<<x<<y<<z<<"):\t" << gv.dd.gGrid_demand[z][x][y];
...
int slidingWindowScan(vector<Inst *> &instList, unsigned rowStartIndex, unsigned colStartIndex, unsigned rowEndIndex, unsigned colEndIndex, unsigned rowSize, unsigned colSize, unsigned rowStep, unsigned colStep,ofstream &logs,unsigned &numCellsMoved,unsigned &numPredictionTrials)
{
	...
	unsigned startGridRowIdx = rowStartIndex, endGridRowIdx = rowSize, startGridColIdx = colStartIndex, endGridColIdx = colSize; // these are for sliding
	...
	while (rowIncr != 0)
	{
		...
		while (colIncr != 0)
		{
			...
			int flag = partitionDesignClustered(instList, dataFile, rowStartIndex, colStartIndex, rowEndIndex, colEndIndex, startGridRowIdx, endGridRowIdx, startGridColIdx, endGridColIdx, cellsInWindow, cellsInRowWindow, cellsBeyondRowWindow,logs,numCellsMoved,numPredictionTrials); 
			if(flag==-1) return 1;
			if (flag == 1)
			{
			...
			// returned flag if (flag == 0) return 0;
...
unsigned pMoveCell(Inst *Cell, unsigned rowBegIdx, unsigned rowEndIdx,
				   unsigned colBegIdx, unsigned colEndIdx,
				   unsigned mCellGridRow, unsigned mCellGridCol, ofstream &logs,unsigned &numCellsMoved,bool &hasMoved,unsigned &wirelengthFlag) // We should change boundary limitd here (since we don't want boundaries to interfere)
{
			...
			if (1) //hpwl < initial_hpwl)	//accepting without hpwl check
			{
				...
				else
				{
					...
					else
					{
						...
						{
							//getDemand();
							commitRouteForCell(Cell, true,(Cell)->netList.end(), logs);
							CellMove *movedCell = new CellMove((Cell),(Cell)->row, (Cell)->col);
							if(hasMoved==false){
								numCellsMoved++;
								...
								//cmListmutex.unlock();
								logs << "Final: " << (Cell)->name<< " moved to " << (Cell)->row << " "<< (Cell)->col << endl;
								hasMoved = true;

							}else{
								gv.dd.cmList.pop_back();
...
/*
int pythonRandomPrediction()
{
    PyObject *pRandomIntFuncObj;
    PyObject *pValue, *pRandomFuncArgObj;
		pRandomIntFuncObj = PyObject_GetAttrString(pRandomFuncObj, "randint");
		// pLoadModelFuncObj is a new reference

	if(pRandomFuncObj !=NULL)
	{

        if (pRandomIntFuncObj && PyCallable_Check(pRandomIntFuncObj))
		{
		    pRandomFuncArgObj = PyTuple_New(2);

			pValue = PyLong_FromLong(1);
			PyTuple_SetItem(pRandomFuncArgObj, 0, pValue);
			pValue = PyLong_FromLong(10);
			PyTuple_SetItem(pRandomFuncArgObj, 1, pValue);
            //Row Value Prediction
				pValue = PyObject_CallObject(pRandomIntFuncObj, pRandomFuncArgObj);
				Y[0]=PyLong_AsLong(pValue);
				printf("Random Value : %d \n",Y[0]);
            //Column Value Prediction
				pValue = PyObject_CallObject(pRandomIntFuncObj, pRandomFuncArgObj);
				Y[1]=PyLong_AsLong(pValue);
				printf("Random Value : %d \n",Y[1]);
            Py_DECREF(pRandomFuncArgObj);
		}
		else
		{
			std::cout << "randint function not available "<<std::endl;
		}
		Py_DECREF(pRandomIntFuncObj);
	}
	else
	{
		printf("Random function not found \n");
	}
}*/
unsigned printRandoms(int lower, int upper)
{
...
/*int brute_force(vector<Inst *> &instList, ofstream &dataFile, unsigned rowStartIndex, unsigned colStartIndex, unsigned rowEndIndex, unsigned colEndIndex,ofstream &logs,unsigned &numCellsMoved,unsigned &numPredictionTrials){
	unsigned cellRow = 0, cellCol = 0;
	for(vector<Inst *>::iterator instListItr = instList.begin(); instListItr != instList.end() ; ++instListItr){
		cellRow = (*instListItr)->row;
		cellCol = (*instListItr)->col;

		unsigned numNets = createNetBasedConnectivityMatrix(cellCount, cellIdxList, net_connectivity_matrix, movableCellList);
		//May write aseparate function too
		// Creat a net connectivity also using a function:-
		// Keep very simple implementation here
		int flag = generateOutputDataSampleCurrentCell(dataFile, rowStartIndex, rowEndIndex, colStartIndex,
													...
													pinIndexList, movableCellList, cellCount, numNets,logs,numCellsMoved,numPredictionTrials);
		if(flag==1) return -1;
	}
	return 1;
}*/
...
{ // ? HEre e pass only the present window after sliding by a predetermined amount
	...
	Inst *currentCell;
	unsigned numPins = 0;
	unsigned net_connectivity_matrix[NUM_CELLS + 1][MAX_NETS + 1];
	for (list<Inst *>::iterator cellIterator = cellsInWindow.begin(); cellIterator != cellsInWindow.end(); cellIterator++)
	{
		...
		if ((currentCell)->adjacencyList.size() < 10)
		{
			...
			{
				// Make the data sample here and pass to the model here only
				unsigned numNets = createNetBasedConnectivityMatrix(cellCount, cellIdxList, net_connectivity_matrix, movableCellList);
				//May write aseparate function too
				// Creat a net connectivity also using a function:-
				// Keep very simple implementation here
										 ...
										 pinIndexes *pinIndexList, list<Inst *> &movableCellList, unsigned k, unsigned numNetsCurrCell,ofstream &logs,unsigned &numCellsMoved,unsigned &numPredictionTrials)
{
	...
	//if (movableCell->cellIndex == FIXED_CELL_IDX)
	{
		...
		// ! generateDataSample uses X but we should ow use 8 different X[]
		//cout << "Rowstart, colstart: " << rowStartIndex << ", " << rowEndIndex << endl;
		//pythonPredictionCode(X,Y);
		//logs << "Predicted values are: " << round(Y[0]) << " and " << round(Y[1]) << endl;
		//predictionLogs << round(Y[0]) << "," << round(Y[1]) << ",";
		//predictionLogs << movableCell->name << ",";
		//<<Y[0]<<","<<Y[1]<<",";
		//<<endl;
		/*numPredictionTrials++;
		...
		unsigned nMCells = 0;
		bool hasMoved = false;
		unsigned wirelengthFlag = -1;
		unsigned endRowValue = rowEndIndex < 10 ? rowEndIndex : endGridRowIdx ;
		unsigned endColValue = colEndIndex < 10 ? colEndIndex : endGridColIdx ;
		for(unsigned m=startGridRowIdx ; m <= endRowValue ; m++){
			for(unsigned n=startGridColIdx ; n <= endColValue ; n++){
				logs << "Predicted values are: " << m << " and " << n << endl;
				predictionLogs << m << ","<< n << ","<<endl;
				predictionLogs << movableCell->name << ","<<m<<","<<n<<","<<endl;
				numPredictionTrials++;
				rowGridStart = rowStartIndex; // gv.rowBeginIdx;
				rowGridEnd = rowEndIndex;	  // gv.rowEndIdx;
				colGridStart = colStartIndex; //  gv.colBeginIdx;
				...
				nMCells = pMoveCell(movableCell, rowGridStart, rowGridEnd, colGridStart, colGridEnd, tempMCellGridRow, tempMCellGridCol, pMoveLogs,numCellsMoved,hasMoved,wirelengthFlag);
		...
		// totalTime += elapsed.count();
		// logs << "Elapsed time is : " << elapsed.count();

		//  if (elapsed.count() > 	180) // 6 hours
		//  {
...
{ // iniitalize variable is passed as first
	...
	for (unsigned i = 1; i <= NUM_CELLS; i++)
	{
		...
		X[inputVectorCountML] = ((initialize == false) ? (double)decimalNumber / 100 : 0);
		inputVectorCountML++;
		decimalNumber = 0;
	}

	inputVectorCountML = 34;
	for (j = 1; j <= k; j++)
	{
	...
	inputVectorCountML = 44;
	for (unsigned lay = 2; lay <= gv.l; lay++)
	{
		for (i = startGridRowIdx; i <= endGridRowIdx; i++)
		{
			for (j = startGridColIdx; j <= endGridColIdx; j++)
			{
				if (initialize == false)
				{
					...
					if (!overFlow)
					{
						...
						// printf("Combiined : %f\n", number);
						X[inputVectorCountML] = ((initialize == false) ? ((float)(gv.dd.gGrid_supply[lay][i][j] - gv.dd.gGrid_demand[lay][i][j]))/10 : 0); //  change has been done here
...
label1:
	...
	inputVectorCountML =244;
	X[inputVectorCountML] = ((initialize == false) ? movedCellLocation : 0);

	inputVectorCountML =245;
	X[inputVectorCountML] = ((initialize == false) ? /*gv.*/ rowPosCell[movedCellLocation] : 0);

	inputVectorCountML = 246;
	X[inputVectorCountML] = ((initialize == false) ? /*gv.*/ colPosCell[movedCellLocation] : 0);

	inputVectorCountML = 247;
	X[inputVectorCountML] = ((initialize == false) ? /*gv.*/ demandValCell[movedCellLocation] : 0);
...
// 			//Row Value Prediction
// 			pValue = PyObject_CallObject(pRandomIntFuncObj, pRandomFuncArgObj);
// 			gv.Y[0] = PyLong_AsLong(pValue);
// 			printf("Random Value : %d \n", gv.Y[0]);
// 			//Column Value Prediction
// 			pValue = PyObject_CallObject(pRandomIntFuncObj, pRandomFuncArgObj);
// 			gv.Y[1] = PyLong_AsLong(pValue);
// 			printf("Random Value : %d \n", gv.Y[1]);
// 			Py_DECREF(pRandomFuncArgObj);
// 		}
// 		else
